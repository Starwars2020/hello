...
podTemplate(
    ...
    node(label) {
        def githubRepository = "https://github.com/Starwars2020/hello.git"
        def githubCredential = "github-api-token"
        def dockerRegistry = "localhost:5000"
        def dockerRegistryCredential = ""
        def dockerImageName = "localhost:5000/hello"
        ...
        stage('Run kubectl') {
            container('kubectl') {
                sh "kubectl apply -f ./k8s/hello-deployment.yaml -n ${nameSpace}"
                sh "kubectl apply -f ./k8s/hello-service.yaml -n ${nameSpace}"
                sh "kubectl apply -f ./k8s/hello-ingress.yaml -n ${nameSpace}"
            }
        }
    }
}